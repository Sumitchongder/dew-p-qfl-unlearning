...
def test_circuit_backend_is_qiskit():
    """The VQC simulates the circuit with genuine Qiskit `Statevector`
    evolution -- there is no alternate simulation backend, so every number
    reported in the paper corresponds to this code path."""
    from src.qflewp import circuit as circuit_module
    assert hasattr(circuit_module, "QuantumCircuit")
    assert hasattr(circuit_module, "Statevector")
    vqc = VQC(n_qubits=4, n_layers=3, n_features=4)
    assert not hasattr(vqc, "backend")
